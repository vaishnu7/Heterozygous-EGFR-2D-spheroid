/*
 * TestPC9spheroid2D_sweep_hyp_onset_rad_multi_exp.hpp
 *
...
{
public:
    void RunSpheroidSimulation(double hypoxia_onset_radius, int sweep_index, unsigned replicate_index)
    {
        std::cout << "\n=== Sweep " << sweep_index << ", Replicate " << replicate_index
                  << ": Hypoxia onset radius = " << hypoxia_onset_radius 
                  << " cell units (~" << hypoxia_onset_radius * 15 << " um) ===" << std::endl;
        ...
        std::string output_dir = "PC9_Spheroid2D_HypoxiaOnsetRadius_" 
        ...
        // SWEPT PARAMETER: Hypoxia onset radius
    ...
    {
        std::cout << "\n=== PC9 Spheroid 2D Cross-Section Parameter Sweep ===" << std::endl;
        std::cout << "Sweeping hypoxia onset radius\n" << std::endl;
        
        // Define hypoxia onset radii
        std::vector<double> hypoxia_onset_radii = {7.0, 8.0, 10.0, 11.0};
        
        // Replicate settings
        unsigned start_rep = 1;
        unsigned num_reps = 5;

        std::cout << "Total runs: " << hypoxia_onset_radii.size() * num_reps << std::endl;

        // Run simulation for each radius x replicate
        for (unsigned i = 0; i < hypoxia_onset_radii.size(); i++)
        {
            for (unsigned rep = start_rep; rep < start_rep + num_reps; rep++)
            {
                RunSpheroidSimulation(hypoxia_onset_radii[i], i + 1, rep);
            }
        }
        
        std::cout << "\n=== Parameter sweep completed! ===" << std::endl;
        std::cout << "4 onset radii x 5 replicates = 20 simulations" << std::endl;
    }
};
