/*
 * TestPC9spheroid2D_sweep_HIF_O2_independent_deg_multi_exp.hpp
 *
...
{
public:
    void RunSpheroidSimulation(double hif_independent_deg, int sweep_index, unsigned replicate_index)
    {
        std::cout << "\n=== Sweep " << sweep_index << ", Replicate " << replicate_index
                  << ": HIF O2 independent degradation rate = " << hif_independent_deg 
                  << " ===" << std::endl;
        ...
        std::string output_dir = "PC9_Spheroid2D_HIF_O2_independent_deg_" 
        ...
        MAKE_PTR_ARGS(HypoxiaSignalingModifier, p_hypoxia_signaling, 
            (0.05,                  // Hypoxia threshold
                ...
                dt,                 // dt
                0.05,               // HIF basal synthesis rate (assumption)
                0.08,               // Max O2 dependent degradation of HIF (assumption)
                hif_independent_deg));            // O2 independent degradation rate of HIF (assumption)
    ...
    void TestPC9SpheroidCrossSection()
    {
        ...
        // Define HIF O2 independent degradation rates to sweep
        std::vector<double> hif_independent_deg = {0.001, 0.002, 0.004, 0.005};
        
        // Replicate settings
        unsigned start_rep = 1;
        unsigned num_reps = 5;

        std::cout << "Total runs: " << hif_independent_deg.size() * num_reps << std::endl;

        // Run simulation for each parameter x replicate
        for (unsigned i = 0; i < hif_independent_deg.size(); i++)
        {
            for (unsigned rep = start_rep; rep < start_rep + num_reps; rep++)
            {
                RunSpheroidSimulation(hif_independent_deg[i], i + 1, rep);
            }
        }
        
        std::cout << "\n=== Parameter sweep completed! ===" << std::endl;
        std::cout << "4 degradation rates x 5 replicates = 20 simulations" << std::endl;
    }
};
